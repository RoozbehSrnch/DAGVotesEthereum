...
# Configuration
BOOTSTRAP_IP = "127.0.0.1"    # Modify the IP to matches the Bootstrap node's Public IP
BOOTSTRAP_PORT = 5000
NODE_PORT = 5001
...
class NormalValidator:
    def __init__(self, port):
        self.ip = "0.0.0.0"
        self.port = port
        self.private_key = AugSchemeMPL.key_gen(str(port).encode().ljust(32, b'\0'))
        self.public_key = self.private_key.get_g1()
        self.validator_id = None
        self.num_validators = None
        self.max_slots = None
        self.slot_duration = None
        self.start_time = None
        self.public_keys = None
        self.block_proposers = None
        ...
        self.chains = [[{"hash": "GENESIS", "parent": None, "payload": "", "aggregation": None, "slot": -1,
                         "proposer_id": 0, "signature": None}]]
        self.current_slot = -1
    ...
    def start_server(self):
        ...
        print(f"Validator listening on {self.ip}:{self.port}")
        ...
        while self.running:
            try:
                ...
                with self.message_lock:
                    ...
                    with print_lock:
                        print(f"Validator {self.validator_id}: Queued message from {sender_id} (client {addr})")
            except Exception as e:
                print(f"Validator {self.validator_id}: Error receiving from {addr}: {e}")
                break
        with print_lock:
            print(f"Validator {self.validator_id}: Connection closed by {addr}")
    ...
    def accept_connections(self):
        while self.running:
            try:
                ...
                with print_lock:
                    print(f"Validator {self.validator_id}: Accepted connection from {addr}")
    ...
    def connect_to_bootstrap(self):
        ...
        self.bootstrap_socket.connect((BOOTSTRAP_IP, BOOTSTRAP_PORT))
        public_key_bytes = bytes(self.public_key)
        self.bootstrap_socket.send(pickle.dumps((public_key_bytes, self.port)))
        print(f"Connected to bootstrap at {BOOTSTRAP_IP}:{BOOTSTRAP_PORT} and sent public key with port {self.port}")

        config_data = self.bootstrap_socket.recv(4096)
        config = pickle.loads(config_data)
        self.validator_id = config["validator_id"]
        ...
        self.public_keys = [G1Element.from_bytes(pk_bytes) for pk_bytes in config["public_keys"]]
        self.peers = config["peers"]
        print(f"Received config: ID={self.validator_id}, Peers={self.peers}")

        start_data = self.bootstrap_socket.recv(4096)
        start_msg = pickle.loads(start_data)
        self.start_time = start_msg["start_time"]
        ...
        for peer_ip, peer_port in self.peers:
            ...
            if addr in self.client_sockets:
                try:
                    ...
                    with print_lock:
                        print(
                            f"Validator {self.validator_id}: Broadcasted to {addr} at time {int(time.time() - self.start_time)}: {message}")
                except Exception as e:
                    print(f"Validator {self.validator_id}: Failed to send to {addr} - {e}")

    def hash_block(self, parent_hash, payload, aggregation, slot, proposer_id):
        data = f"{parent_hash}{payload}{aggregation if aggregation else ''}{slot}{proposer_id}"
        ...
        while time.time() < deadline:
            try:
                ...
                if msg_type == "block":
                    ...
                    computed_hash = self.hash_block(block["parent"], block["payload"], block["aggregation"],
                    ...
                    if block["aggregation"]:
                        agg = block["aggregation"]
                        voter_pks = [self.public_keys[i] for i, bit in enumerate(agg["bitfield"]) if bit == 1]
                        agg_sig = agg["aggregated_signature"]
                        messages = [agg["block_hash"].encode()] * len(voter_pks)
                        if not AugSchemeMPL.aggregate_verify(voter_pks, messages, agg_sig):
                            with print_lock:
                                print(
                                    f"Validator {self.validator_id}: Aggregate signature verification failed in block")
                            continue
                    ...
                    agg_message = f"{block_hash}{agg['aggregated_signature']}{''.join(map(str, agg['bitfield']))}".encode()
                    if "aggregator_signature" not in agg:
                        continue
                    ...
                    voter_pks = [self.public_keys[i] for i, bit in enumerate(agg["bitfield"]) if bit == 1]
                    agg_sig = agg["aggregated_signature"]
                    messages = [block_hash.encode()] * len(voter_pks)
                    if not AugSchemeMPL.aggregate_verify(voter_pks, messages, agg_sig):
                        continue
                    verification_time = time.time() - verification_start
                    ...
                    self.aggregations.append((slot, generator_id, block_hash, agg))
            except queue.Empty:
                break
    ...
    def create_block(self):
        while self.running:
            ...
            if not self.has_proposed and self.current_slot > 0 and \
                ...
                valid_aggregations = [
                    agg_entry for agg_entry in self.aggregations
                    if agg_entry[0] == prev_slot and agg_entry[2] == parent_hash
                ]

                if valid_aggregations:
                    best_agg = max(valid_aggregations, key=lambda x: sum(x[3]["bitfield"]), default=None)
                    candidates = [agg for agg in valid_aggregations if
                                  sum(agg[3]["bitfield"]) == sum(best_agg[3]["bitfield"])]
                    selected_agg = random.choice(candidates)[3] if candidates else None
                    aggregation = {
                        "block_hash": selected_agg["block_hash"],
                        "aggregated_signature": selected_agg["aggregated_signature"],
                        "bitfield": selected_agg["bitfield"]
                    }

                block_hash = self.hash_block(parent_hash, payload, aggregation, slot, proposer_id)
        ...
        while self.running:
            ...
            if not self.has_aggregated and self.validator_id in self.aggregators[self.current_slot]:
                ...
                if votes_for_block:
                    ...
                    agg_message = f"{target_block}{aggregated_signature}{''.join(map(str, bitfield))}".encode()
                    aggregator_signature = self.sign(agg_message)
                    aggregation = {
    ...
    def update_chains(self, block):
        ...
        if parent_hash not in self.pending_chain_requests:
            ...
            self.broadcast({
                "type": "request_chain",
                "data": {"root_block": parent_hash, "end_block": block["hash"]}
            })
    ...
    def calculate_branch_weight(self, block_hash, tree, for_voting, votes_dict, visited=None):
        ...
        weight = 0 if self.current_slot == 0 and for_voting else sum(
            1 for vote in votes_dict.values() if vote["block_hash"] == block_hash)
    ...
    def print_tree_weights(self, for_voting):
        ...
        weights = {block_hash: self.calculate_branch_weight(block_hash, tree, for_voting, votes_dict) for block_hash in
                   all_blocks}
    ...
    def best_chain(self, for_voting=False):
        if not self.chains:
            return [
                {"hash": "GENESIS", "parent": None, "payload": "", "aggregation": None, "slot": -1, "proposer_id": 0,
                 "signature": None}]
        ...
        while self.running and time.time() <= deadline:
            ...
            if self.current_slot != prev_slot:
                with print_lock:
                    ...
                    print(
                        f"Validator {self.validator_id}: block proposer of the slot: {'None' if self.current_slot == 0 else self.block_proposers[proposer_idx]}")
    ...
    def run(self):
        ...
        accept_thread = threading.Thread(target=self.accept_connections, daemon=True,
                                         name=f"Accept_{self.validator_id}")
        ...
        message_thread = threading.Thread(target=self.process_messages, daemon=True,
                                          name=f"MsgProc_{self.validator_id}")
...
if __name__ == "__main__":
    validator = NormalValidator(NODE_PORT)
    validator.run()
