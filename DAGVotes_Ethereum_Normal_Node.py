...
class NormalValidator:
    def __init__(self, port):
        ...
        self.chains = [[{"hash": "GENESIS", "parent": None, "payload": "", "evidence": None, "slot": -1, "proposer_id": 0, "signature": None}]]
        self.current_slot = -1
    ...
    def timekeeper(self):
        ...
        print(f"Validator {self.validator_id}: Average aggregation verification time: {sum(self.agg_verification_times)/len(self.agg_verification_times):.6f} seconds")
        print(f"Validator {self.validator_id}: Average aggregation creation time: {sum(self.agg_creation_times)/len(self.agg_creation_times):.6f} seconds")
        self.running = False
